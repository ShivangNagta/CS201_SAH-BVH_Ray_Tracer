...
// Add procedural noise function here (placeholder)
// Implementing Perlin or Simplex noise would be ideal.
...
SDL_Color trace_ray(Ray ray, Sphere *spheres, int num_spheres, Ground ground, Vec3 light_dir, int depth) {
    ...
    if (hit_sphere != -1 || closest_t < INFINITY) {
        ...
        Vec3 reflect_dir = vec3_reflect(light_dir, hit_normal);
        intensity += pow(fmax(vec3_dot(view_dir, reflect_dir), 0), spheres[hit_sphere].specular);
        ...
        color.r = (Uint8)fmin(color.r * intensity, 255);
        color.g = (Uint8)fmin(color.g * intensity, 255);
        color.b = (Uint8)fmin(color.b * intensity, 255);
...
#endif
{
    ...
    while (running) {
        ...
        render(renderer, spheres, NUM_SPHERES, ground, light_dir, WIDTH, HEIGHT, camera_position);
