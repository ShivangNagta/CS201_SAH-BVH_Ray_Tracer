...
#endif
{
    ...
    SDL_Window *window = SDL_CreateWindow("Ray Tracer with Movable Camera", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 800, 600, SDL_WINDOW_SHOWN);
    if (window == NULL) {
    ...
    for (int i = 0; i < NUM_SPHERES; i++) {
        spheres[i] = (Sphere){
            .center = {random_float(-5, 5), random_float(0.5f, 5), random_float(-5, 5)},
    ...
    while (running) {
        ...
        render(renderer, spheres, NUM_SPHERES, ground, light_dir, 800, 600, camera_position);
